...
async def Video_info(video_file):
    # Take a screenshot from ig video for thumbnail
...
async def Take_screen_shot(video_file):
    # Take a screenshot from ig video for thumbnail
    clip = VideoFileClip(video_file)
    s_time = random.randint(1, round(clip.duration))
    out_path = f"{video_file}.jpg"
    clip.save_frame(out_path, t=s_time)
    return out_path
